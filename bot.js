...
async function detectBookingEndpoint(event) {
  ...
  for (const endpoint of POSSIBLE_BOOK_ENDPOINTS) {
    try {
      ...
      const payload = new URLSearchParams();
      payload.set("event_id", String(event.id));
      payload.set("participant_type", "all");
      payload.set("qty", "1");
      ...
      if (res.status === 200 && res.data) {
        ...
        if (!response.includes("unknown AJAX route")) {
          console.log(`   ✅ Endpoint ditemukan: ${endpoint}`);
          DETECTED_BOOK_ENDPOINT = endpoint;
          return endpoint;
        } else {
          console.log(`   ❌ ${endpoint} - unknown route`);
        }
      }
    } catch (err) {
...
function buildPayloadJSON(event) {
  // SESUAIKAN field dengan Network tab saat booking manual!
  return {
    event_id: event.id,
    participant_type: "all",
    qty: 1,
    // Contoh jika diwajibkan backend:
    // npm: "1234567",
    // name: "Nama Lengkap",
...
function buildPayloadForm(event) {
  ...
  p.set("event_id", String(event.id));
  p.set("participant_type", "all");
  p.set("qty", "1");
  // p.set("npm", "1234567");
  // p.set("name", "Nama Lengkap");
  // p.set("email", "kamu@domain.com");
  // p.set("phone", "08xxxxxxxxxx");
  return p;
}
